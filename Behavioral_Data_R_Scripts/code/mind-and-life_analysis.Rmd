...
```{r, message = FALSE}
# Function to perform t-test, calculate effect size, and print results
perform_t_test <- function(variable_name, data) {
  ...
  mean_lkm <- mean(lkm_data, na.rm = TRUE)
  sd_lkm <- sd(lkm_data, na.rm = TRUE)
  mean_pmr <- mean(pmr_data, na.rm = TRUE)
  sd_pmr <- sd(pmr_data, na.rm = TRUE)
  overall_mean <- mean(c(lkm_data, pmr_data), na.rm = TRUE) # Calculate overall mean
  ci <- t_test$conf.int
  
  # Calculate Cohen's d for effect size
  n_lkm <- length(lkm_data)
  n_pmr <- length(pmr_data)
  pooled_sd <- sqrt(((n_lkm - 1) * var(lkm_data, na.rm = TRUE) + (n_pmr - 1) * var(pmr_data, na.rm = TRUE)) / (n_lkm + n_pmr - 2))
  cohen_d <- (mean_lkm - mean_pmr) / pooled_sd
  
  cat("\nTesting Variable:", variable_name, "\n")
  cat("Mean in LKM:", mean_lkm, "\n")
  cat("SD in LKM:", sd_lkm, "\n")
  cat("Mean in PMR:", mean_pmr, "\n")
  cat("SD in PMR:", sd_pmr, "\n")
  cat("Overall Mean:", overall_mean, "\n") # Print overall mean
  cat("Test Statistic:", test_statistic, "\n")
  cat("P-value:", p_value, "\n")
  cat("95% CI for the difference in means:", ci, "\n")
  cat("Effect Size (Cohen's d):", cohen_d, "\n")
}

# Variables to test
variables <- c("socialConnectedness_mean_T1","loneliness_mean_T1", "traitEmpathyIRI_EC_mean_T1", "traitEmpathyIRI_PT_mean_T1", "traitEmpathyIRI_fant_mean_T1", "traitEmpathyIRI_PD_mean_T1", "inclusionOtherSelf_mean_T1", "wellbeing_mean_T1")

# Perform t-tests for each variable
for (variable in variables) {
  perform_t_test(variable, dfB_noT3)
}
...
# Optional: Save the final table as a CSV
write.csv(desc_stats_with_corr, "spearman_correlation_table_Full Sample_all_with_pvalues_T1.csv", row.names = FALSE)
...
fitH1c_1_contrast <- lme(loneliness_mean ~ 1 + time*GroupID + age + Male + Other + YesCollegeDegree, 
                         random = ~1 + time | SubID, 
                         data = dfB_all_long, 
                         method = "ML", 
...
# save model 
tab_model(fitH1c_1_contrast, file = "fit1H1c_T3.html", 
          show.df = T, 
          show.aic = T, 
          show.se = T, 
          show.stat = T, 
...
write.csv(results_apa, "APA_t_test_results.csv", row.names = FALSE)
print(results_apa)
