...
# Usage: ./run_1stlevel_unsmoothed.sh -s 031
...
for condition in self other; do
    ...
    for run in "${runs[@]}"; do
        ...
        # SCALE AND SMOOTH
        3dTstat -prefix ${OUTPUT_PATH}/sub-${SUBID}/preproc/sub-${SUBID}_task-empathy_run-${run_index}_space-MNI152NLin2009cAsym_desc-preproc_mean.nii.gz -overwrite "/my/path/fMRIData/subjects/derivatives/sub-${SUBID}/func/sub-${SUBID}_task-empathy_run-${run_index}_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz[3..292]"
        3dcalc -a "/my/path/fMRIData/subjects/derivatives/sub-${SUBID}/func/sub-${SUBID}_task-empathy_run-${run_index}_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz[3..292]" -b ${OUTPUT_PATH}/sub-${SUBID}/preproc/sub-${SUBID}_task-empathy_run-${run_index}_space-MNI152NLin2009cAsym_desc-preproc_mean.nii.gz -expr 'min(200, a/b*100)*step(a)*step(b)' -prefix ${OUTPUT_PATH}/sub-${SUBID}/preproc/sub-${SUBID}_task-empathy_run-${run_index}_space-MNI152NLin2009cAsym_desc-preproc_scaled.nii.gz -overwrite

        # FIRSTLEVEL GLM for each run and condition
        3dDeconvolve \
            -overwrite \
            -jobs 6 \
            -input ${OUTPUT_PATH}/sub-${SUBID}/preproc/sub-${SUBID}_task-${task_id}_run-${run_index}_space-MNI152NLin2009cAsym_desc-preproc_scaled.nii.gz \
            -polort A \
            -num_stimts 14 \
            ...
            -x1D ${OUTPUT_PATH}/sub-${SUBID}/sub-${SUBID}_task-${task_id}_glm-${glm_id}_space-MNI152NLin2009cAsym_unsmoothed_xmat.1D \
            -bucket ${OUTPUT_PATH}/sub-${SUBID}/sub-${SUBID}_task-${task_id}_glm-${glm_id}_space-MNI152NLin2009cAsym_unsmoothed_stats.nii.gz \
            -bucket ${OUTPUT_PATH}/sub-${SUBID}/sub-${SUBID}_task-${task_id}_glm-${glm_id}_space-MNI152NLin2009cAsym_unsmoothed_stats.BRIK
