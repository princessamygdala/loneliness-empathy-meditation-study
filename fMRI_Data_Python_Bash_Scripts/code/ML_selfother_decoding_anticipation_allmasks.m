...
for k = 1:length(KOsubjects)
    ...
    for m = 1:length(mask_files)
        ...
        cfg.results.dir = sprintf('/my/path/MVPAresults_anticipation/%s/%s', mask_name, cursub);
        if ~exist(cfg.results.dir, 'dir'), mkdir(cfg.results.dir); end
        
        % Handle beta files (compressed and uncompressed)
        beta_loc_uncompressed = sprintf('/my/path/3dLSS/%s/%s_task-empathy_self-other-anticipation-combined_space-MNI152NLin6Asym_LSS+tlrc.BRIK', cursub, cursub);
        beta_loc_compressed = sprintf('/my/path/3dLSS/%s/%s_task-empathy_self-other-anticipation-combined_space-MNI152NLin6Asym_LSS+tlrc.BRIK.gz', cursub, cursub);
        ...
        labelname1a = 'self_pain_anticipation*';
        labelname2a = 'self_neutral_anticipation*';
        labelname1b = 'other_anticipation*';
        labelname2b = 'other_neutral_anticipation*';
