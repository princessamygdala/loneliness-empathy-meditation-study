...
def infotodict(seqinfo):
    ...
    for s in seqinfo:
        ...
        * image_type
        ...
        # SELF
        if ('Thumb' in s.protocol_name) and ('2' in s.series_description) and (s.total_files_till_now > 730):
            info[task_self] = [s.series_id]

        # OTHER
        if ('Thumb' in s.protocol_name) and ('1' in s.series_description) and (s.total_files_till_now > 19):
            info[task_other] = [s.series_id]

        
        # FIELD MAP
        if ('gre_field_mapping' in s.series_id) and ('ND' in s.series_description) and (s.total_files_till_now <18):
            info[mag] = [s.series_id]

        if ('gre_field_mapping' in s.series_id) and ('ND' in s.series_description) and (s.total_files_till_now >17):
