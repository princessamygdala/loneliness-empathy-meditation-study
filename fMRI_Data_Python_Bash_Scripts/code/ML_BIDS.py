...
def infotodict(seqinfo):
    ...
    for s in seqinfo:
        ...
        * image_type
        ...
        # ANATOMICAL 
        if ('MPRAGE' in s.protocol_name) and ('ND' not in s.series_description):
        ...
        if ('self-pain_field_map' == s.series_description) and ('ND' not in s.series_description):
            fmap_info['self'].append(s)

        if ('other-pain_field_map' == s.series_description) and ('ND' not in s.series_description):   
