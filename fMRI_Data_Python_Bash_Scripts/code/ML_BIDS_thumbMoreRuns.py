...
# it's called thumb because the task was originally named thumbtask
...
def infotodict(seqinfo):
    ...
    for s in seqinfo:
        ...
        * image_type
        ...
        # SELF
        if ('Thumb' in s.protocol_name) and ('2' in s.series_description) and (s.total_files_till_now > 315):
            info[task_self] = [s.series_id]

        # OTHER
        if ('Thumb' in s.protocol_name) and ('1' in s.series_description) and (s.total_files_till_now > 730):
            info[task_other] = [s.series_id]

        
        # FIELD MAP
        if ('gre_field_mapping' in s.series_id) and (s.total_files_till_now <9):
            info[mag] = [s.series_id]

        if ('gre_field_mapping' in s.series_id) and (s.total_files_till_now >8):
