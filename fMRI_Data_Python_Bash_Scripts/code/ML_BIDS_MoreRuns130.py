...
def infotodict(seqinfo):
    ...
    for s in seqinfo:
        ...
        * image_type
        ...
        # RUN SELF
        if ('self-pain' in s.series_description) and ('field_map' not in s.protocol_name) and (s.total_files_till_now > 19):
            info[task_self] = [s.series_id]

        # RUN OTHER
        if ('other-pain' in s.series_description) and ('field_map' not in s.protocol_name) and (s.total_files_till_now > 638):
            info[task_other] = [s.series_id]

        # GET FMAP INFO, BUT DON"T ADD TO INFO YET
        if ('self-pain_field_map' == s.series_description) and ('ND' in s.series_description):
            fmap_info['self'].append(s)

        if ('other-pain_field_map' == s.series_description) and ('ND' in s.series_description):   
